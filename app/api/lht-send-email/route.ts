/**
 * API Route: Send LHT Video Upload Email to Instructors
...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            console.log(`[lht-send-email] Sending email to ${instructorEmails.length} instructors`);
            console.log(`[lht-send-email] To: ${emailOptions.to}`);
            if (emailOptions.bcc && emailOptions.bcc.length > 0) {
                console.log(`[lht-send-email] BCC: ${emailOptions.bcc.join(', ')}`);
            }

            await transporter.sendMail(emailOptions);

            console.log(`[lht-send-email] Email sent successfully to ${instructorEmails.length} instructors`);
            console.log(`[lht-send-email] Recipients: ${instructorEmails.join(', ')}`);
            ...
            console.error('[lht-send-email] Email sending error:', {
        ...
        console.error('[lht-send-email] Error:', errorMessage);
        return NextResponse.json(
            { error: errorMessage },
            { status: 500 }
        );
