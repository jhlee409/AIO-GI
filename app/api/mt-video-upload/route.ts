/**
 * API Route: MT Video Upload Metadata Handler
 * 
 * 리팩토링된 구조:
...
export async function POST(request: NextRequest) {
    ...
    // 요청 시작 로깅
    console.log(`[mt-video-upload:${requestId}] START`, {
    ...
    try {
        ...
        try {
            ...
            console.log(`[mt-video-upload:${requestId}] Request body parsed`, {
            ...
            console.error(`[mt-video-upload:${requestId}] JSON parse error:`, {
        ...
        if (!userEmail || !position || !name || !hospital || !videoUrl || !fileName) {
            ...
            console.error(`[mt-video-upload:${requestId}] Missing required fields:`, {
                missingFields,
                received: {
        ...
        if (fileSize && fileSize > 100 * 1024 * 1024) {
            console.error(`[mt-video-upload:${requestId}] File too large:`, {
                fileSize,
        ...
        console.log(`[mt-video-upload:${requestId}] Finding instructors...`, { hospital });
        ...
        try {
            ...
            console.log(`[mt-video-upload:${requestId}] Instructors found`, {
            ...
            console.warn(`[mt-video-upload:${requestId}] Failed to find instructors:`, {
                message: errorMessage,
        ...
        console.log(`[mt-video-upload:${requestId}] Creating log file...`);
        const logFileName = `${position}-${name}-MT`;
        try {
            ...
            console.log(`[mt-video-upload:${requestId}] Log file created`, {
                logFileName,
                duration: `${logDuration}ms`,
            });
        } catch (logError: any) {
            const errorMessage = extractErrorMessage(logError, 'Failed to create log file');
            console.error(`[mt-video-upload:${requestId}] Failed to create log file:`, {
                message: errorMessage,
        ...
        console.log(`[mt-video-upload:${requestId}] SUCCESS`, {
            videoUrl,
        ...
        console.error(`[mt-video-upload:${errorId}] ERROR:`, {
            message: errorMessage,
            stack: error.stack,
            name: error.name,
            code: error.code,
