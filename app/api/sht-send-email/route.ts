/**
 * API Route: Send SHT Video Upload Email to Instructors
...
export async function POST(request: NextRequest) {
    try {
        console.log('[sht-send-email] Request received');
        const bodyData = await request.json();
        const { instructors, subject, body } = bodyData;
        
        console.log('[sht-send-email] Request data:', {
        ...
        try {
            ...
            console.log(`[sht-send-email] Sending email to ${instructorEmails.length} instructors`);
            console.log(`[sht-send-email] To: ${emailOptions.to}`);
            if (emailOptions.bcc && emailOptions.bcc.length > 0) {
                console.log(`[sht-send-email] BCC: ${emailOptions.bcc.join(', ')}`);
            }

            await transporter.sendMail(emailOptions);

            console.log(`[sht-send-email] Email sent successfully to ${instructorEmails.length} instructors`);
            console.log(`[sht-send-email] Recipients: ${instructorEmails.join(', ')}`);
            ...
            console.error('[sht-send-email] Email sending error:', {
        ...
        console.error('[sht-send-email] Error:', errorMessage);
        return NextResponse.json(
            { error: errorMessage },
            { status: 500 }
        );
