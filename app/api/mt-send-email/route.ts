/**
 * API Route: Send MT Video Upload Email to Instructors
...
export async function POST(request: NextRequest) {
    try {
        console.log('[mt-send-email] Request received');
        const bodyData = await request.json();
        const { instructors, subject, body } = bodyData;
        
        console.log('[mt-send-email] Request data:', {
        ...
        try {
            ...
            console.log(`[mt-send-email] Sending email to ${instructorEmails.length} instructors`);
            console.log(`[mt-send-email] To: ${emailOptions.to}`);
            if (emailOptions.bcc && emailOptions.bcc.length > 0) {
                console.log(`[mt-send-email] BCC: ${emailOptions.bcc.join(', ')}`);
            }

            await transporter.sendMail(emailOptions);

            console.log(`[mt-send-email] Email sent successfully to ${instructorEmails.length} instructors`);
            console.log(`[mt-send-email] Recipients: ${instructorEmails.join(', ')}`);
            ...
            console.error('[mt-send-email] Email sending error:', {
        ...
        console.error('[mt-send-email] Error:', errorMessage);
        return NextResponse.json(
            { error: errorMessage },
            { status: 500 }
        );
